...
describe('eventData', function(){
  ...
  it('should issue a GET request to /data/event/11 when GetEvent is called with id 11',
  ...
  inject(function(eventData, $httpBackend){
    $httpBackend.when('GET', '/data/event/11')
      .respond({name: 'My Event'});
  ...
  it('should set id to 999', inject(function(eventData, $httpBackend){
  ...
  it('should issue a GET requst to data/event when getAllEvents is called',
  inject(function(eventData, $httpBackend){
    $httpBackend.when('GET', '/data/event')
      .respond([{name: 'My Event'}]);
